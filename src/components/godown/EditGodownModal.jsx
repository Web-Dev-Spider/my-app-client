...
const EditGodownModal = ({ godown, allGodowns, onClose, onSuccess }) => {
    const [name, setName] = useState(godown.name);
    const [code, setCode] = useState(godown.code || '');
    const [address, setAddress] = useState(godown.address || '');
    const [contact, setContact] = useState(godown.contact || '');
    const [notes, setNotes] = useState(godown.notes || '');
    const [isActive, setIsActive] = useState(godown.isActive);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState('');

    // Check if this is the only active godown
    const activeCount = allGodowns.filter(g => g.isActive).length;
    const isOnlyActiveGodown = isActive && activeCount === 1;
    ...
    const handleSubmit = async (e) => {
        ...
        // Check if trying to deactivate the only active godown
        if (godown.isActive && !isActive && activeCount === 1) {
            setError('Cannot deactivate the only active godown');
            return;
        }
        ...
        try {
            const response = await api.put(`/inventory/stock-locations/${godown._id}`, {
                name: name.trim(),
            ...
            if (response.data.success) {
                ...
                setError(response.data.message || 'Failed to update godown');
            }
        } catch (err) {
            setError(err.response?.data?.message || 'Error updating godown');
            console.error(err);
        } finally {
            setLoading(false);
        }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-3">
            <div className="bg-theme-secondary rounded-xl shadow-2xl w-full max-w-2xl border border-theme-color max-h-[90vh] overflow-y-auto">
                ...
                <div className="sticky top-0 flex items-center justify-between p-4 md:p-5 border-b border-theme-color bg-theme-tertiary">
                    <h2 className="text-lg md:text-xl font-bold text-theme-primary">Edit Godown</h2>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-4 md:p-5 space-y-4">
                    ...
                    {isOnlyActiveGodown && (
                        <p className="text-xs text-yellow-600 bg-yellow-50 border border-yellow-200 rounded px-3 py-2">
                            ⚠️ This is the only active godown. At least one must remain active.
                        </p>
                    )}
